...
export class ProgressController {
    ...
    async getProgressPercentageById(req, res) {
        ...
        try {
            ...
            const { id, idType } = dto;
            const endpoint = STACKBY_ENDPOINTS_HASHTABLE[idType];
            if (!endpoint) {
                return res.status(STATUS_CODE.BAD_REQUEST).json({
            ...
            if (idType === IdType.THEME_ID) {
                ...
                const result = await this.progressService.getProgressPercentageById({ id, idType, userId }, totalItems, themes, topics);
    ...
    async saveStatusProgress(req, res) {
    ...
    async getTopicExercisesStatusProgress(req, res) {
        const dto = plainToInstance(ProgressDTO, req.params);
        const userId = req.user?.id;
        try {
            await validateOrReject(dto);
    ...
    async getExerciseStatusProgress(req, res) {

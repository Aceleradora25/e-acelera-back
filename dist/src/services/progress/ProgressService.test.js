...
describe("ProgressService", () => {
    ...
    describe("getProgressPercentageById", () => {
        it("retorna progresso 0 se totalItens for 0", async () => {
            const result = await progressService.getProgressPercentageById({ id: "1", idType: IdType.TOPIC_ID, userId: 1 }, 0);
        ...
        it("calcula corretamente o progresso quando há itens completados", async () => {
            ...
            const result = await progressService.getProgressPercentageById({ id: "1", idType: IdType.TOPIC_ID, userId: 1 }, 10);
        ...
        it("lança erro se o count falhar", async () => {
            ...
            await expect(progressService.getProgressPercentageById({ id: "1", idType: IdType.TOPIC_ID, userId: 1 }, 5)).rejects.toThrow("Error fetching user progress from database");
        });
        it("retorna {progress: 0, topics: []} se theme não encontrado", async () => {
            const themes = { data: [] };
            const topics = { data: [] };
            const result = await progressService.getProgressPercentageById({ id: "notfound", idType: IdType.THEME_ID, userId: 1 }, 10, themes, topics);
        ...
        it("retorna progresso 0 se topicsInfo está vazio", async () => {
            ...
            const topics = { data: [] };
            const result = await progressService.getProgressPercentageById({ id: "theme1", idType: IdType.THEME_ID, userId: 1 }, 10, themes, topics);
            expect(result).toEqual({ progress: 0, topics: [] });
        });
        it("calcula progresso de theme com tópicos e exercícios", async () => {
            ...
            const result = await progressService.getProgressPercentageById({ id: "theme1", idType: IdType.THEME_ID, userId: 1 }, 3, themes, topics);
    ...
    describe("getSingleStatusProgressByItemId", () => {
        it("retorna o progresso correto para o itemId e userId", async () => {
            ...
            const result = await progressService.getSingleStatusProgressByItemId("item1", 1);
            expect(result).toEqual(mockProgress);
        });
        it("lança erro quando findFirst falhar", async () => {
            prismaMock.progress.findFirst.mockRejectedValue(new Error("DB error"));
            await expect(progressService.getSingleStatusProgressByItemId("item1", 1)).rejects.toThrow("Error fetching user progress from database");
        });
    });
    describe("getAllStatusProgressById", () => {
        ...
        it("lança erro quando findMany falhar", async () => {
            ...
            await expect(progressService.getAllStatusProgressById({
                id: "item1",
                idType: IdType.TOPIC_ID,
                userId: 1,
    ...
    describe("saveStatusProgress", () => {
        ...
        it("lança erro quando o upsert falha", async () => {
            ...
            await expect(progressService.saveStatusProgress({
                elementType: ElementType.Video,
                itemId: "item123",
                itemStatus: ItemStatus.Completed,
                themeId: "theme123",

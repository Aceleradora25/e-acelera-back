...
export class ExerciseController {
    ...
    async updateExerciseStatus(req: Request, res: Response) {
        ...
        try {
            ...
            if (!currentProgress) {
                return res.status(STATUS_CODE.NOT_FOUND).json({ message: "Progress not found for the exercise" })
            }
    
            if (currentProgress.itemStatus === itemStatus) {
                return res.status(STATUS_CODE.OK).json({ message: "Status value is already being used" });
            }
            ...
            return res.status(STATUS_CODE.INTERNET_SERVER_ERROR).json({ message: "Internal server error while processing the request" })
        }
    }
    ...
    async getTopicExercisesStatus(req: Request, res: Response) {
        ...
        try {
            ...
            if (getAllExercisesStatus.length === 0) {
                return res.status(STATUS_CODE.NOT_FOUND).json({ message: "Progress not found" })
            }
    ...
    try {
        ...
        if (!topicId) {
            return res.status(STATUS_CODE.BAD_REQUEST).json({ message: "topicId not found" })
        }
        ...
        if (exerciseStatus.length === 0){
            return res.status(STATUS_CODE.NOT_FOUND).json({ message: "Status not found"})
        }
